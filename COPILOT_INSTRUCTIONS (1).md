...
A persistent top-level nav (visible on every page) with three tabs: **Itinerary**
(`/`), **Destinations** (`/destinations`), and **Info** (`/info`).
...
- `/destinations/:slug` — one destination's full overview, photo, and its
  ...
  **recommendation sheet** (see below) rather than navigating to a separate page —
  ...
  `body` text and photo.
- `/info` — a static reference page (not a sheet, a real page — this content is
  meant to be read start-to-finish or skimmed by section, not previewed and
  dismissed) built from `data/info.json`. Render each `section` as a labeled block
  (e.g. a heading plus its `items`), each `item` showing its `title` and `body`.
  No photos needed here — this is reference text, not a place list, so skip the
  Pixabay integration for this page entirely. A simple table-of-contents or
  jump-links list at the top (one per section title) is a nice-to-have given how
  many sections this page has, but not required.
...
- At build time, import all files in `data/destinations/` (e.g. with
  ...
  removed.
- `data/info.json` — content for the `/info` page. Shape:
  ```
  {
    "pageTitle": string,
    "sections": [
  ...
  ```
  Render sections in the order they appear in the file — that order is intentional
  (weather/packing first since it's the most immediately actionable, logistics and
  language later).
...
**Info page styling note:** This page is text-heavy reference content, not a
visual/photo experience like the rest of the site — still use the same color
tokens and typography (section titles in the display face, body text in the body
face) for consistency, but don't force in photos or the ticket/route motifs here;
a clean, readable, well-spaced list of sections is the right fit for this one page.
...
│   │   └── InfoPage.jsx
│   ├── components/
│   │   ├── NavTabs.jsx           (persistent top-level nav: Itinerary /
│   │   │                          Destinations / Info)
